...
class CodeScanner:
    ...
    def __init__(self, args) -> None:
        self.args = args
        # Default to huggingface if provider is not specified
        provider = getattr(args, 'provider', 'huggingface')
        ...
        self.provider = init_provider(
            provider, model, host, port, token, endpoint
        )
    ...
    def _scan_changes(self):
        ...
        """
        try:
            # If we have a valid repo and PR number, get changed files in the PR
            ...
            # If we have a valid repo but no PR number, or if we're scanning a local directory
            else:
    ...
    def _is_pr_number_valid(self):
        return self.args.pr_number is not None and self.args.pr_number > 0
