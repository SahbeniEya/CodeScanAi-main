...
class JSONVulnerabilityScanner:
    ...
    def _scan_generic_json(self, file_path: str, data: dict):
        ...
        self._check_sensitive_keys(file_path, data, sensitive_keys)

    def _check_sensitive_keys(self, file_path: str, data: Any, sensitive_keys: List[str], path: str = ""):
        """
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                ...
                self._check_sensitive_keys(file_path, value, sensitive_keys, new_path)

        elif isinstance(data, list):
            for i, item in enumerate(data):
                new_path = f"{path}[{i}]"
                self._check_sensitive_keys(file_path, item, sensitive_keys, new_path)


def scan_json_files(directory: str, nvd_api_key: Optional[str] = None) -> List[Vulnerability]:
    """
    ...
    Returns:
        List[Vulnerability]: List of detected vulnerabilities.
    """
    scanner = JSONVulnerabilityScanner(nvd_api_key)
    vulnerabilities = []
